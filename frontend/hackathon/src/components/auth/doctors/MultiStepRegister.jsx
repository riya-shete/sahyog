...
const MultiStepRegister = ({ role = 'doctor', onBack }) => {
  ...
  return (
    <form onSubmit={handleRegister} className="flex flex-col gap-4 p-6 max-w-md w-full mx-auto">
      <div className="flex gap-2">
        {[1, 2, 3].map((s) => (
          <div key={s} className={`flex-1 h-1 rounded ${step >= s ? 'bg-blue-600' : 'bg-gray-300'}`} />
        ))}
      </div>
      ...
      {step === 2 && (
        <>
          ...
          <div className="flex justify-between">
            <button type="button" onClick={() => setStep(1)} className="underline text-sm">Back</button>
            <button
              type="button"
              onClick={() => {
                if (!email || !phone) {
        ...
        <>
          <div className="relative">
            <input
              ...
              required
            />
            <span className="absolute inset-y-0 right-3 flex items-center cursor-pointer text-gray-600" onClick={() => setShowPassword((prev) => !prev)}>
          ...
          <div className="relative">
            <input
              ...
              required
            />
            <span className="absolute inset-y-0 right-3 flex items-center cursor-pointer text-gray-600" onClick={() => setShowRePassword((prev) => !prev)}>
              {showRePassword ? <EyeSlashIcon className="w-5 h-5" /> : <EyeIcon className="w-5 h-5" />}
            </span>
          </div>

          {password && rePassword && password !== rePassword && (
            <p className="text-sm text-red-600 mt-[-10px] -mb-2">Passwords do not match</p>
          )}

          <div className="flex justify-between">
            <button type="button" onClick={() => setStep(2)} className="underline text-sm">Back</button>
            <button
              type="submit"
      ...
      <p className="text-sm text-center mt-2">
        ...
        <button type="button" onClick={onBack} className="text-blue-600 underline">Login</button>
      </p>
    </form>
  )
}
